...
export function InsightsMetrics(props: InsightsMetricsProps) {
  const { records, navigate, setSearchQuery, searchQuery } = useAppContext();
  ...
  const errorRate = totalCount > 0 ? ((failedCount / totalCount) * 100).toFixed(2) : "0.00";
  const processingSpeed = totalCount > 0 ? Math.round(totalCount * 19.2) : 0;
    ...
    <>
      ...
      <nav className="hidden md:flex flex-col h-screen w-64 left-0 top-0 bg-surface-container-low dark:bg-surface-container-low border-r border-outline-variant dark:border-outline-variant p-md gap-base shrink-0 fixed">
        ...
        <div className="flex flex-col gap-xs flex-grow">
          <a
            className="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant hover:text-on-surface hover:bg-surface-variant dark:hover:bg-surface-variant transition-all duration-150 cursor-pointer select-none"
          ...
          {/* Active Tab */}
          <a
            className="flex items-center gap-md px-md py-sm rounded-lg bg-secondary-container dark:bg-secondary-container text-on-secondary-container dark:text-on-secondary-container cursor-pointer select-none"
        ...
        <header className="hidden md:flex justify-between items-center h-14 w-full px-margin ml-auto bg-background sticky top-0 z-30">
          ...
          <div className="flex items-center gap-md">
            ...
            <img
              ...
              onClick={() => navigate("profile")}
              data-alt="A small, circular profile picture of a young professional man with short dark hair, smiling slightly, set against a blurred background. The image is used as a user avatar in a modern, dark-themed UI."
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuDM86qgtlP-WWqqa0QRr96MlHyEDGWooVTbUAG6yQtg0kCVbfu3ZRMHwlE3qR7wsJLCs_UzMG8bGPNkak08C4VZnyeqQa3GBZvZl5sD7CeaQWm0-rF1OGz50em9Qpf881vzbqbEFF2rx4JEvf4IQNL_FG3pPb5fYTdkfBp1A6uMj-wm6hDBEQrLv90gyLShjRa9WbbrOLWgcU9HJUgIN-KX9Nl18BOazjEwaJdy43g1sGt-TbDVUZwZzmYwNoKbV66578YAKfVUHZY"
            />
          </div>
        </header>
        {/* Main Canvas */}
        <main className="flex-1 p-margin flex flex-col gap-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-lg">
            ...
            <div className="col-span-1 lg:col-span-4 flex flex-col gap-lg">
              ...
              <div className="bg-surface rounded-lg border border-outline-variant p-md flex-1 flex flex-col justify-center">
                ...
                <div className="text-[32px] leading-tight font-bold text-on-surface tracking-tight mb-xs">
                  {processingSpeed} <span className="text-headline-sm font-headline-sm text-outline-variant font-normal">ops/min</span>
                </div>
              ...
              <div className="flex flex-col gap-md">
                <div>
                  ...
                  <div className="w-full bg-surface-container-high rounded-full h-2">
                    <div className="bg-primary-container h-2 rounded-full" style={{width: totalCount > 0 ? `${(activeCount / totalCount) * 100}%` : "0%"}}></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-body-sm font-body-sm mb-xs">
                    <span className="text-on-surface">Pending Review</span>
                    <span className="text-on-surface-variant">{pendingCount}</span>
                  </div>
                  <div className="w-full bg-surface-container-high rounded-full h-2">
                    <div className="bg-secondary-container h-2 rounded-full" style={{width: totalCount > 0 ? `${(pendingCount / totalCount) * 100}%` : "0%"}}></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-body-sm font-body-sm mb-xs">
                  ...
                  <div className="w-full bg-surface-container-high rounded-full h-2">
                    <div className="bg-error-container h-2 rounded-full" style={{width: totalCount > 0 ? `${(failedCount / totalCount) * 100}%` : "0%"}}></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-1 lg:col-span-6 bg-surface rounded-lg border border-outline-variant">
              ...
              <div className="flex flex-col">
                {records.length === 0 ? (
                  <div className="px-md py-sm text-body-sm font-body-sm text-on-surface-variant">No recent activity</div>
                ) : (
                  records.slice(0, 3).map((record) => (
                    <div key={record.id} className="px-md py-sm border-b border-outline-variant/50 hover:bg-surface-variant transition-colors flex items-center gap-md">
                      ...
                      <div className="flex-1">
                        <div className="text-body-sm font-body-sm text-on-surface">{record.entityName}</div>
                        <div className="text-label-sm font-label-sm text-outline">{record.lastUpdated}</div>
                      </div>
                      <span className={`text-label-sm font-label-sm px-2 py-0.5 rounded ${
                        record.status === "active"
                          ? "text-primary-container bg-surface-tint/10"
                          : record.status === "failed"
                          ? "text-error bg-error-container/20"
                          : "text-on-surface-variant bg-surface-variant"
                      }`}>
                        {record.status === "active" ? "Success" : record.status === "failed" ? "Alert" : "Log"}
                      </span>
                    </div>
                  ))
                )}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-surface-container border-t border-outline-variant flex justify-around items-center h-16 z-40 pb-safe">
        <a
          className="flex flex-col items-center justify-center w-full h-full text-on-surface-variant hover:text-on-surface"
          onClick={() => navigate("dashboard")}
        >
