...
import type { OpsRecord } from "../types/domain";

interface DashboardProps {}

export function Dashboard(props: DashboardProps) {
  const { navigate, records, searchQuery, setSearchQuery, filterStatus, setFilterStatus, selectRecord } = useAppContext();
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "operations-export.json";
    a.click();
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "active":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase bg-[#10b981]/10 text-[#10b981] border border-[#10b981]/20">
            Active
          </span>
        );
      case "pending":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase bg-tertiary-container/20 text-tertiary border border-tertiary/20">
            Pending
          </span>
        );
      case "completed":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase bg-outline-variant/30 text-on-surface-variant border border-outline-variant/50">
            Completed
          </span>
        );
      case "failed":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase bg-error-container/20 text-error border border-error/20">
            Failed
          </span>
        );
      default:
  ...
  return (
    <>
      ...
      {/* Active Nav Item */}
      <a className="bg-secondary-container dark:bg-secondary-container text-on-secondary-container dark:text-on-secondary-container rounded-lg flex items-center gap-sm px-sm py-sm cursor-pointer select-none" href="#" onClick={(e) => { e.preventDefault(); }}>
      <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>dashboard</span>
      <span className="text-label-md font-label-md">Dashboard</span>
      </a>
      <a className="text-on-surface-variant hover:text-on-surface hover:bg-surface-variant dark:hover:bg-surface-variant transition-all duration-150 rounded-lg flex items-center gap-sm px-sm py-sm cursor-pointer select-none" href="#" onClick={(e) => { e.preventDefault(); handleNav('insights'); }}>
      <span className="material-symbols-outlined">analytics</span>
      <span className="text-label-md font-label-md">Insights</span>
      </a>
      <a className="text-on-surface-variant hover:text-on-surface hover:bg-surface-variant dark:hover:bg-surface-variant transition-all duration-150 rounded-lg flex items-center gap-sm px-sm py-sm cursor-pointer select-none" href="#" onClick={(e) => { e.preventDefault(); handleNav('settings'); }}>
      ...
      <a className="text-on-surface-variant hover:text-on-surface hover:bg-surface-variant dark:hover:bg-surface-variant transition-all duration-150 rounded-lg flex items-center gap-sm px-sm py-sm cursor-pointer select-none" href="#" onClick={(e) => { e.preventDefault(); handleNav('dashboard'); }}>
      ...
      <button
        className="p-sm text-on-surface-variant hover:bg-surface-bright dark:hover:bg-surface-bright transition-colors duration-200 cursor-pointer active:opacity-80 rounded-full md:hidden"
        type="button"
      ...
      <div>
      <div className="text-headline-lg font-headline-lg text-on-surface">{activeCount.toLocaleString()}</div>
      ...
      <div>
      <div className="text-headline-lg font-headline-lg text-on-surface">{pendingCount.toLocaleString()}</div>
      ...
      <td className="p-md">{statusBadge(record.status)}</td>
      ...
      <span className="pl-sm">
        Showing {filteredRecords.length > 0 ? (safePage - 1) * pageSize + 1 : 0}-{Math.min(safePage * pageSize, filteredRecords.length)} of {filteredRecords.length}
      </span>
      <div className="flex items-center gap-xs">
      <button
        className={`w-8 h-8 rounded flex items-center justify-center ${safePage <= 1 ? "cursor-not-allowed opacity-50" : "hover:bg-surface-variant"}`}
        type="button"
      ...
      <button
        key={page}
        className={`w-8 h-8 rounded flex items-center justify-center ${page === safePage ? "bg-secondary-container text-on-secondary-container" : "hover:bg-surface-variant"}`}
        type="button"
      ...
      <button
        className={`w-8 h-8 rounded flex items-center justify-center ${safePage >= totalPages ? "cursor-not-allowed opacity-50" : "hover:bg-surface-variant"}`}
        type="button"
